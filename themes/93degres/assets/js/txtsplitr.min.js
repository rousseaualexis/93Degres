	...
	for(var i = 0;i<this.HTMLobjects.length;i++){
		...
		if(this.vars.type.indexOf("chars")!=-1){
			function splitChars(parent,st){
				...
				parent.innerHTML = startTag + spliddit(splid).join(endTag+startTag) + endTag;
				//parent.innerHTML = startTag + splidditparent.innerHTML.replace(/&\w+;/g,"ህ").split("").join(endTag+startTag) + endTag;
